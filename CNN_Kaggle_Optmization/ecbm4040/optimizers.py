...
class SGDmomentumOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for i,j in model.params.items():
            velocitys[i] = momentum*velocitys[i] + learning_rate * grads[i]
            params[i] = params[i] - velocitys[i]
...
class RMSpropOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for i,j in model.params.items():
            cache[i]=cache[i]*gamma+(1-gamma)*(grads[i]**2)
            params[i]=params[i]-grads[i]*learning_rate/(np.sqrt(cache[i]+eps))
        model.params=params
        self.cache=cache
        #model.grads=grads
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for i,j in model.params.items():
            ...
            momentums[i]=beta1*momentums[i]+(1-beta1)*(grads[i])
            velocitys[i]=beta2*velocitys[i]+(1-beta2)*(grads[i]**2)
            
            momentum_avg=momentums[i]/(1-beta1**t)
            velo_avg=velocitys[i]/(1-beta2**t)
            params[i]=params[i]-learning_rate*(momentum_avg)/(np.sqrt(velo_avg)+eps)
