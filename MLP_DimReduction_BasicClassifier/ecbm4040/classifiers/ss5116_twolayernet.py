...
class ss5116_TwoLayerNet(object):
    """
    ...
    def __init__(self, input_dim=3072, hidden_dim=200, num_classes=10, reg=0.0, weight_scale=1e-2):
        ...
        self.velocity=[0. for i in range(4)] # Velocity for SGD
    def loss(self, X, y):
        """
        ...
        self.X=X
        self.y=y
        layer1_output=self.layer1.feedforward(X)
        layer2_output=self.layer2.feedforward(layer1_output)
        loss,dx=softmax_loss(layer2_output,y)
    ...
    def step(self, learning_rate=1e-5):
        ...
        #cache=[0 for i in enumerate(grads)]
        
        #Trial- Nesterov momentum but low accuracy
        '''for i in range(len(params)):
            v_prev = self.velocity[i] # back this up
            self.velocity[i] = mu * self.velocity[i] + learning_rate * grads[i] 
            params[i] += mu * v_prev - (1 + mu) * self.velocity[i]
        '''
        #SGD with momentum
        mu=0.00004
    ...
    def predict(self, X):
        ...
        #######################################################
        #out=layer2.feedforward(layer1.feedforward(X))
        predictions=np.argmax(layer2.feedforward(layer1.feedforward(X)),axis=1)
        #######################################################
        #                 END OF YOUR CODE                    #
        #######################################################
